...
description: Open a pull request adding a local MonoField plugin to the MonoField community catalog using gh CLI.
triggers:
  - contribute plugin
  - open design pr
  - github pull request
...
# Contribute Plugin to MonoField

Use this workflow when the active project contains a copied plugin folder and the user wants to propose it for the MonoField community catalog.
...
Keep the pull request focused. Do not modify unrelated MonoField files unless a manifest validation issue requires a tiny supporting change.
