...
Give this file to a coding agent when you want it to create or improve an MonoField plugin.

## Mission

Create a portable MonoField plugin that can:

1. Run as a normal Agent Skill through `SKILL.md`.
2. Install into MonoField through `open-design.json`.
3. Be validated locally.
4. Be published as an independent open source repo or submitted as a PR to MonoField.
...
- `plugins/spec/PUBLISHING-REGISTRIES.md` when the user asks to publish outside MonoField
...
3. Keep the `SKILL.md` portable. It may mention MonoField behavior, but the core workflow must still make sense in any Agent Skills compatible agent.
