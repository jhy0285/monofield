...
这是可移植 MonoField 插件的精简契约。完整产品规范仍以 `docs/plugins-spec.zh-CN.md` 为准；本文档面向贡献者和外部编码 agent。
...
当插件需要出现在 MonoField marketplace 卡片或 starter 中时，添加 `open-design.json`：
...
- `name` 是稳定插件 id。
- `specVersion` 是此 manifest 遵循的 MonoField 插件规范版本。除非 schema 升级，否则使用当前规范包的值（`1.0.0`）。
...
外部 registry 分发策略见 [`PUBLISHING-REGISTRIES.zh-CN.md`](PUBLISHING-REGISTRIES.zh-CN.md)。简言之：把 GitHub 或 MonoField PR 作为 source of truth，让文件夹能作为通用 `SKILL.md` skill 安装；本地验证通过后，再发布或登记到 skills.sh、ClawHub 或其他 registry。
