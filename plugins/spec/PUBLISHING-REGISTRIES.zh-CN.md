...
MonoField 插件刻意设计成一个文件夹可以跨多个 agent 生态流转。最稳妥的发布模型是：

1. 把公开 GitHub 仓库或 MonoField PR 作为 source of truth。
2. 保持 `SKILL.md` 可移植、适合 registry 读取。
3. 添加 `open-design.json` 作为 MonoField sidecar。
...
1. 本地验证插件文件夹。
2. 推送公开 GitHub 仓库，或向 MonoField 打开 PR。
3. 在 README 中添加 MonoField 和通用 Agent Skills 客户端的安装说明。
...
| 目标 | 适合场景 | 源形态 | 发布策略 |
| --- | --- | --- | --- |
| MonoField | OD marketplace、composer chips、pipelines、GenUI、artifact provenance | `SKILL.md` + `open-design.json` | 向 MonoField 提 PR，或发布指向插件仓库的 marketplace index entry。 |
...
普通 MonoField 插件应优先走这个路径，因为它们以 `SKILL.md` 为核心。
...
- 链回 canonical GitHub repo 和 MonoField PR。
- changelog 诚实且版本化。
