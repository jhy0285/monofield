...
MonoField plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an MonoField PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `open-design.json` as the MonoField sidecar.
...
2. Push a public GitHub repository or open an MonoField PR.
3. Add README install instructions for MonoField and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| MonoField | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `open-design.json` | Open a PR to MonoField or publish a marketplace index entry pointing to the plugin repo. |
...
Use this path for normal MonoField plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and MonoField PR.
