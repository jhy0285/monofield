...
当你希望一个编码 agent 创建或改进 MonoField 插件时，可以把这个文件直接交给它。

## 任务目标

创建一个可移植的 MonoField 插件，使它可以：

1. 通过 `SKILL.md` 作为普通 Agent Skill 运行。
2. 通过 `open-design.json` 安装到 MonoField。
3. 在本地完成校验。
4. 发布为独立开源仓库，或作为 PR 提交给 MonoField。
...
- 当用户要求发布到 MonoField 以外的 registry 时，阅读 `plugins/spec/PUBLISHING-REGISTRIES.zh-CN.md`
...
3. 保持 `SKILL.md` 可移植。它可以提到 MonoField 行为，但核心 workflow 必须在任何 Agent Skills 兼容 agent 中都能理解。
