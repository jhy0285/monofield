...
const TO_HELP_BY_PLATFORM: Record<ToolPackPlatform, string> = {
  ...
  win: "build target: all|dir|nsis|store|zip (default: nsis). `store` produces an unsigned AppX for Partner Center; `zip` produces a portable zip; `all` produces dir+nsis+zip.",
};
