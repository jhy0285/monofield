...
export async function buildWinLauncherPayloadArchive(
  ...
  const writeOverlay = async (input: { includeExecutable: boolean }): Promise<void> => {
    ...
    if (input.includeExecutable) {
      await cp(join(builtApp.unpackedRoot, "MonoField.exe"), join(overlayRoot, "payload", "MonoField.exe"));
    }
    await writeFile(
...
export async function validateWinLauncherPayloadArchive(input: {
  ...
  try {
    ...
    requirePayloadManifestValue(manifest.entry?.executable, "entry.executable", "payload/MonoField.exe");

    await stat(join(extractRoot, archiveRelativePath("payload/MonoField.exe")));
