...
describe("resolveMacInstallIdentity", () => {
  it("keeps stable builds on the canonical mac identity", () => {
    expect(resolveMacInstallIdentity(makeConfig("/work", "release-stable"))).toMatchObject({
      ...
      installerTitle: "MonoField",
      productName: "MonoField",
      publicAppBundleName: "MonoField.app",
      systemAppBundleName: "MonoField.app",
    });
  });
  ...
  it("uses first-class beta app identity for beta release namespaces", () => {
    ...
    expect(resolveMacInstallIdentity(config)).toEqual({
      appId: "io.monofield.desktop.beta",
      executableName: "MonoField Beta",
      installerTitle: "MonoField Beta",
      productName: "MonoField Beta",
      publicAppBundleName: "MonoField Beta.app",
      systemAppBundleName: "MonoField Beta.app",
    });
    expect(resolveMacPaths(config).appPath).toMatch(/MonoField Beta\.app$/);
  });
  ...
  it("uses first-class preview app identity for preview release namespaces", () => {
    ...
    expect(resolveMacInstallIdentity(config)).toEqual({
      appId: "io.monofield.desktop.preview",
      executableName: "MonoField Preview",
      installerTitle: "MonoField Preview",
      productName: "MonoField Preview",
      publicAppBundleName: "MonoField Preview.app",
      systemAppBundleName: "MonoField Preview.app",
    });
    expect(resolveMacPaths(config).appPath).toMatch(/MonoField Preview\.app$/);
  });
  ...
  it("uses first-class prerelease app identity for prerelease release versions and namespaces", () => {
    ...
    expect(resolveMacInstallIdentity(prereleaseVersionConfig)).toEqual({
      ...
      installerTitle: "MonoField Prerelease",
      productName: "MonoField Prerelease",
      publicAppBundleName: "MonoField Prerelease.app",
      systemAppBundleName: "MonoField Prerelease.app",
    });
    expect(resolveMacPaths(prereleaseVersionConfig).appPath).toMatch(/MonoField Prerelease\.app$/);
