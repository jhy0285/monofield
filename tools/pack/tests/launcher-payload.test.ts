...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "MonoField-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "MonoField-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "MonoField Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "MonoField Beta", "MonoField.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "MonoField-release-beta-win-payload.7z"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "MonoField Beta", "Uninstall.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "MonoField.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "MonoField.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "MonoField Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/MonoField Beta.app",
        executable: "payload/MonoField Beta.app/Contents/MacOS/MonoField Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
    ...
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/MonoField.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("MonoField Beta.app");
      expect(manifest.entry.executable).toBe("payload/MonoField Beta.app/Contents/MacOS/MonoField Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/MonoField.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "MonoField.exe"));
