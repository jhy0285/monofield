...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "MonoField-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "MonoField-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "MonoField"),
    installedExePath: join(namespaceRoot, "runtime", "install", "MonoField", "MonoField.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "MonoField-second-payload.7z"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "MonoField", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "MonoField.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "MonoField.exe"))).resolves.toMatchObject({
