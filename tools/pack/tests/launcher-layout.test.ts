...
describe("tools-pack launcher layout", () => {
  ...
  it("resolves platform payload archive and extraction paths without changing installed app paths", () => {
    ...
    expect(macPayload.archivePath).toBe(join(mac.roots.output.namespaceRoot, "payload", "MonoField-release-beta-payload.zip"));
    expect(winPayload.archivePath).toBe(join(win.roots.output.namespaceRoot, "payload", "MonoField-release-beta-win-payload.7z"));
  ...
  it("exposes a stable mac payload zip path next to existing mac artifacts", () => {
    ...
    expect(paths.payloadZipPath).toBe(join(config.roots.output.namespaceRoot, "payload", "MonoField-release-beta-payload.zip"));
    expect(paths.dmgPath).toBe(join(config.roots.output.namespaceRoot, "dmg", "MonoField-release-beta.dmg"));
    expect(paths.zipPath).toBe(join(config.roots.output.namespaceRoot, "zip", "MonoField-release-beta.zip"));
  });
