...
// everything it needs to install MonoField, expose it as MCP, and
...
export interface AgentGuideOptions {
  ...
  daemonUrl?: string;
  /**
   * Optional `monofield` binary path / hint. When provided we mention it in the
   * setup checklist so the agent knows whether to run `monofield …` directly or
   * spawn the packaged binary.
   */
  cliHint?: string;
  /** Optional MonoField version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# MonoField — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants MonoField wired into your workflow. MonoField is a local ' +
      'privileged daemon (`monofield`) plus a Skills + Design-Systems registry that exposes ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported MonoField version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the MonoField app (packaged).');
  lines.push('');
  lines.push('2. Detect available agent CLIs and confirm `monofield` is on PATH:');
  lines.push('');
  lines.push('   ```bash');
  lines.push('   monofield doctor');
  lines.push('   monofield status --json');
  lines.push('   ```');
  if (options.cliHint) {
    lines.push('');
    lines.push(`   The user reported \`monofield\` at: \`${options.cliHint}\``);
  }
  lines.push('');
  ...
  lines.push('   monofield skills list --json');
  lines.push('   ```');
  lines.push('');
