...
describe('TokenUsageDisclosure', () => {
  it('shows response metrics and their measurement source in Korean', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '사용량: 실측' }));
    ...
    expect(screen.getByText(/전체 모델 컨텍스트/)).toBeTruthy();
  });
  ...
  it('explains the approximation only for estimated usage', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '사용량: 추정' }));
  ...
  it('shows measured response coverage for the conversation', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '이번 대화 누적: 측정 불가' }));
