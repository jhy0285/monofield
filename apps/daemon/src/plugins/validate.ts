...
// DoctorReport shape the post-install `monofield plugin doctor <id>`
// command emits.
//
// The lift from `monofield plugin doctor`:
...
//
// Rationale: the author tooling ships scaffold, export, and publish commands.
// `monofield plugin validate` closes the loop: the author can run lint
...
export interface ValidatePluginFolderInput {
  ...
  // `monofield.json` / `SKILL.md` / `.claude-plugin/plugin.json`
