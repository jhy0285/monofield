// Phase 4 / spec §14.1 — `monofield plugin publish --to <catalog>` PR-template launcher.
//
...
//   - open-design            → jhy0285/monofield (plugins/community/<plugin-name>/).
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      const bodyWithRegistry = [
        body,
        '',
        '## MonoField registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/monofield.json`',
        '- Generated index: `plugins/registry/community/monofield-marketplace.json`',
        '- Required checks: `monofield plugin validate`, `monofield plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
      const url = newIssueUrl('jhy0285/monofield', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'jhy0285/monofield',
        url,
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `monofield.json` sidecar (additive MonoField metadata).');
  lines.push('');
  lines.push('Generated by `monofield plugin publish` — see the MonoField repository documentation.');
  return lines.join('\n');
}
