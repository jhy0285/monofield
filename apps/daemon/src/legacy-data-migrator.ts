 ...
 *
 * MonoField 0.3.x ran from the repo and wrote runtime state to
 ...
 * way to know about it. See https://github.com/jhy0285/monofield/issues/710.
 *
 * This module gives operators a recovery path. When `MONOFIELD_LEGACY_DATA_DIR`
 ...
 * @see https://github.com/jhy0285/monofield/issues/710
 */
import * as fs from 'node:fs';
import * as path from 'node:path';

export interface MigrateLegacyDataDirOptions {
  /** Path to the legacy `.od/` directory (typically MONOFIELD_LEGACY_DATA_DIR). */
 ...
 */
function isDiscardableLegacyTempPath(candidate: string): boolean {
  return path.basename(candidate) === '.tmp';
}

function assertNoSymlinks(srcRoot: string, displayPath = srcRoot): void {
  // Project-local .tmp directories contain regenerable build output and may
  // legitimately include package-manager symlinks. They are not user data,
  // so leave the entire subtree behind instead of blocking a safe migration.
  if (isDiscardableLegacyTempPath(srcRoot)) return;
...
function stagePayload(legacyDir: string, stagingDir: string): string[] {
  ...
  for (const entry of PAYLOAD_ENTRIES) {
    ...
    fs.cpSync(src, dst, {
      ...
      force: true,
      errorOnExist: false,
      filter: (candidate) => !isDiscardableLegacyTempPath(candidate),
    });
    copied.push(entry);
  }
  return copied;
...
export function migrateLegacyDataDirSync(
  ...
  const log = options.logger ?? {
    info: (m) => console.log(`[monofield-migrate] ${m}`),
    warn: (m) => console.warn(`[monofield-migrate] ${m}`),
  };

  const raw = options.legacyDir;
  if (raw === undefined || raw.length === 0) {
    return { status: 'noop', reason: 'MONOFIELD_LEGACY_DATA_DIR not set' };
  }
  const legacyDir = path.resolve(raw);
  const dataDir = path.resolve(options.dataDir);

  if (legacyDir === dataDir) {
    return { status: 'noop', reason: 'legacy and current data directories are identical' };
  }
  ...
  if (!legacyDirHasPayload(legacyDir)) {
    ...
    throw new LegacyMigrationError(
      'legacy_dir_invalid',
      `MONOFIELD_LEGACY_DATA_DIR="${legacyDir}" is not a usable legacy data dir (expected app.sqlite directly inside it). Quit MonoField, fix the path, and relaunch.`,
    );
  }
  ...
  if (existing.length > 0) {
    throw new LegacyMigrationError(
      'data_dir_not_empty',
      `MONOFIELD_DATA_DIR="${dataDir}" already contains payload entries (${existing.join(', ')}); refusing to merge legacy data on top. Move the existing data aside or pick a fresh data root before re-running with MONOFIELD_LEGACY_DATA_DIR.`,
    );
  }
  ...
  const stagingDir = path.join(
    ...
    `${path.basename(dataDir)}.monofield-migrate-${process.pid}-${Date.now()}`,
  );
