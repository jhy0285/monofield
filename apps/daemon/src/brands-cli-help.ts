// Help surface for `monofield brand`. Kept pure and separate from cli.ts so a test can
...
export const BRAND_USAGE = `Usage:
  monofield brand list [--json]               List extracted brands (id, name, domain, status).
  monofield brand extract <url> [--json]      Start an agent-driven brand extraction. Reserves the
  monofield brand create <url> [--json]       brand and a backing project with the site open in a
  ...
  monofield brand preview <id> [--json]       Re-render brand.html from the project's current
  ...
  monofield brand finalize <id> [--json]      Register the agent's extracted kit (brand.json in the
                                       backing project) as a design system; marks it ready.
                                       --project <projectId> overrides the backing project.
                                       --locale <locale> overrides the stored brand locale.
  monofield brand extract-from-html <id> --html-file <path|->
  ...
  monofield brand get <id> [--json]           Print one brand's full detail (meta + brand + guide).
  monofield brand delete <id> [--json]        Remove a brand and its registered design system.
