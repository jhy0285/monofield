...
MonoField-owned media execution is **disabled for this run**. Do not call
\`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate\`, Codex built-in imagegen, MonoField media
...
shell command through \`MONOFIELD_NODE_BIN\` + \`MONOFIELD_BIN\` is HOW you actually produce bytes.
...
- \`MONOFIELD_NODE_BIN\`    — absolute path to the Node-compatible runtime that started the daemon. Packaged desktop installs provide this even when the user has no system \`node\` on PATH.
- \`MONOFIELD_BIN\`         — absolute path to the MonoField CLI script. On POSIX shells run with \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" …\`.
...
\`\`\`bash
"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate \\
...
For media projects, \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate …\` is the **only**
...
before the first generate attempt. Treat \`MONOFIELD_NODE_BIN\`, \`MONOFIELD_BIN\`,
...
to completion by calling \`media wait <taskId>\` through \`MONOFIELD_NODE_BIN\` + \`MONOFIELD_BIN\`
...
- \`exit 2\` — still **running**. Final stdout line is
  ...
  \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media wait <taskId> --since <n>\` to continue from where you left
...
\`\`\`bash
out=\$("$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate --surface image --model flux-pro-ultra --prompt "…")
ec=\$?
if [ "\$ec" -ne 0 ]; then
  echo "\$out" >&2; exit "\$ec"
fi
...
- **video**:   ${VIDEO_IDS}
  ...
  \`--image <project-relative-path>\` to \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate\`. The
...
\`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate\` as-is via \`--model <id>\`;
...
2. **Dispatch immediately when the brief is complete.** For image and video
   ...
   **Do ask** if the output intent is genuinely ambiguous (e.g. "make something cool"
   ...
   \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate --surface video --model hyperframes-html --composition-dir <rel>\`.
...
3. **Generate by shell, reply in one short message.** When you invoke
   \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate\`, do it inside a clearly-labelled tool call.
...
4. **Iterate by re-running.** To revise, call \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate\` again
...
1. **HTTP status.** When stubs are disabled (the default release-build
   ...
   placeholder instead.
2. **Exit code.** \`"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" media generate\` exits \`0\` for
   both immediate completion and successful queued/running handoff; inspect
   the final stdout JSON for either \`file\` or \`taskId\`. \`"$MONOFIELD_NODE_BIN"
   "$MONOFIELD_BIN" media wait\` exits \`0\` on terminal **done**, \`2\` when the
