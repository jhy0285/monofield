...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```bash',
    `"$MONOFIELD_NODE_BIN" "$MONOFIELD_BIN" research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```powershell',
    `& $env:MONOFIELD_NODE_BIN $env:MONOFIELD_BIN research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```cmd',
    `"%MONOFIELD_NODE_BIN%" "%MONOFIELD_BIN%" research search --query "<search query>" --max-sources ${maxSources}`,
