...
export function configuredAllowedOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env.MONOFIELD_ALLOWED_ORIGINS ?? env.OD_ALLOWED_ORIGINS ?? '';
  if (!raw.trim()) return [];
  return raw
    .split(',')
    ...
    .map((origin) => {
      ...
      if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
        throw new Error('MONOFIELD_ALLOWED_ORIGINS only supports http:// and https:// origins');
      }
...
export function allowedBrowserPorts(
  ...
  const webPort = Number(env.MONOFIELD_WEB_PORT ?? env.OD_WEB_PORT);
...
export function isLocalSameOrigin(
  ...
  const bindHost = env.MONOFIELD_BIND_HOST ?? env.OD_BIND_HOST ?? '127.0.0.1';
  ...
  if (origin == null || origin === '') {
    ...
    // MONOFIELD_ALLOWED_ORIGINS unreachable for legitimate same-origin GETs
  ...
  // origin in MONOFIELD_ALLOWED_ORIGINS. Trust the Origin header in that case:
