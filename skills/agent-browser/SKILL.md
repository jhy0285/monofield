...
description: |
  ...
  MonoField browser-tab context, testing web apps, dogfooding MonoField
  previews, QA, bug hunts, or reviewing app quality. Prefer local MonoField
...
Use `agent-browser` for local MonoField preview validation: inspect rendered
...
For selected MonoField browser tabs and browser-use/browser-harness-style
...
5. If the selected MonoField context only provided a URL/title and no browser
...
Do not run MonoField's own daemon CLI as a browser automation tool. Commands
...
With the MonoField preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `MonoField`, current URL under `127.0.0.1:17573`,
visible MonoField UI text in the snapshot, and a screenshot at
