...
name: ui-ux-pro-max
description: |
  Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in MonoField.
triggers:
  - "ui ux patterns"
  - "design patterns"
  - "ux heuristics"
...
## Current MonoField scope

MonoField currently ships this entry as discovery metadata only. If this `SKILL.md`
...
If those upstream files are not installed, explain that MonoField only has the
catalog entry for this skill and ask whether to continue with MonoField's
