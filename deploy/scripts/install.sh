...
# ---------------------------------------------------------------------------
DEFAULT_PORT=7456
DEFAULT_IMAGE="ghcr.io/jhy0285/monofield:latest"
...
printf "${BOLD}  │${RESET}   ${CYAN}◈${RESET}  ${BOLD}MonoField${RESET}                     ${BOLD}│${RESET}\n"
...
# ---------------------------------------------------------------------------
# 2b. Check if MonoField is already running
# ---------------------------------------------------------------------------
if $CONTAINER_CMD ps --filter "name=monofield" --format '{{.Names}}' 2>/dev/null | grep -q 'monofield'; then
  STATUS="$($CONTAINER_CMD inspect --format '{{.State.Status}}' monofield 2>/dev/null || echo 'unknown')"
  IMAGE="$($CONTAINER_CMD inspect --format '{{.Config.Image}}' monofield 2>/dev/null || echo 'unknown')"
  PORTS="$($CONTAINER_CMD port monofield 2>/dev/null || echo 'unknown')"

  error "MonoField is already running."
  printf "\n"
  printf "  Container:  monofield\n"
  printf "  Status:     %s\n" "$STATUS"
  printf "  Image:      %s\n" "$IMAGE"
  printf "  Ports:      %s\n" "$PORTS"
  printf "\n"
...
MONOFIELD_IMAGE=${IMAGE}
MONOFIELD_PORT=${PORT}
MONOFIELD_ALLOWED_ORIGINS=${ALLOWED_ORIGINS}
MONOFIELD_MEM_LIMIT=${MEM_LIMIT}
NODE_OPTIONS=--max-old-space-size=192
MONOFIELD_API_TOKEN=${GENERATED_TOKEN}
ENVFILE
...
if [ "$OS" = "Linux" ] && [ "$OPT_NO_SYSTEMD" = "0" ]; then
  if command -v systemctl >/dev/null 2>&1; then
    ...
    SYSTEMD_UNIT="${SYSTEMD_DIR}/monofield.service"
    ...
    {
      echo "[Unit]"
      echo "Description=MonoField daemon (${CONTAINER_RUNTIME} Compose)"
    ...
    systemctl --user enable monofield 2>/dev/null || true
    ok "systemd unit installed: monofield.service"
    step "Manage with: systemctl --user start|stop|status monofield"
  else
...
printf "  Image:        %s\n" "$IMAGE"
printf "  Data volume:  monofield_data\n"
printf "  Config:       %s\n" "$ENV_FILE"
if [ "$OS" = "Linux" ] && [ -f "${HOME}/.config/systemd/user/monofield.service" ]; then
  printf "  Service:      systemd (monofield.service)\n"
fi
printf "\n"
printf "  Next steps:\n"
