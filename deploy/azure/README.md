...
Deploy MonoField to Microsoft Azure from the published runtime image — the
same single Alpine image used by [`deploy/docker-compose.yml`](../docker-compose.yml)
and the [Helm chart](../../charts/monofield). The daemon serves both the API
and the built web UI on one port, so there is no separate web container.

> [!IMPORTANT]
> **These lanes are for evaluation and demos, not durable data.** MonoField
> stores its state in a SQLite database under `/app/.monofield`, and SQLite needs real
...
> (named volume) or the [Helm chart](../../charts/monofield) (PVC with
...
Both run as a single instance (MonoField uses single-writer SQLite).
...
- An API token. Generate one with `openssl rand -hex 32`; the daemon requires
  `MONOFIELD_API_TOKEN` and will not start without it.
...
```bash
az group create --name monofield-rg --location eastus
...
| --- | --- |
| `name` (`monofield`) | Base name; a unique suffix is appended to globally-scoped resources |
| `location` (resource group location) | Azure region |
| `image` (`ghcr.io/jhy0285/monofield:latest`) | Container image; pin to a digest for production |
...
The deployed app's own origin is always added to `MONOFIELD_ALLOWED_ORIGINS`, and
`MONOFIELD_PUBLIC_BASE_URL` is set to that origin so OAuth callbacks resolve correctly.
...
deploy/azure/deploy-azure.sh \
  --target app-service \
  --resource-group monofield-rg \
  --image ghcr.io/jhy0285/monofield@sha256:<digest>
```
...
- **ACI** exposes the daemon's port directly over plain HTTP with no managed
  TLS. Access is still gated by `MONOFIELD_API_TOKEN`, and browser traffic by
  `MONOFIELD_ALLOWED_ORIGINS`, but treat this lane as evaluation / trusted-network use.
  For production with HTTPS, use App Service or place ACI behind Application
  Gateway / Front Door.
- Keep `MONOFIELD_API_TOKEN` secret. It is passed as a secure parameter (and a secure
...
```bash
az group delete --name monofield-rg --yes --no-wait
```
